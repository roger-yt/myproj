...
def calc_all_metrics(ref_model, tokenizer, xz_leftpad, xz_mask, y_rightpad, y_mask, stop_strings, stop_tokens):
    ...
    for one_xz, one_xz_mask, one_y, one_y_mask in zip(xz_leftpad, xz_mask, y_rightpad, y_mask):
        ...
        with torch.no_grad():
            # Generate
            # seq = ref_model.generate(input_ids=new_prompt_toks['input_ids'].to(ref_model.device), 
            #                         attention_mask=new_prompt_toks['attention_mask'].to(ref_model.device), 
            ...
            gen_txt = tokenizer.decode(seq[0][len(new_prompt_toks['input_ids'][0]):], skip_special_tokens=True)
            true_ans = y_txt[len('The answer is'):-1] # remove the final period
            # gen_ans = gen_txt[:len(true_ans)]
            if true_ans==None or len(true_ans) == 0:
                prompt_gen_acc = torch.FloatTensor([0])
            else:
                try:
                    while true_ans[0] == " " and len(true_ans) > 1:
                        true_ans = true_ans[1:]
                except:
                    print("true_ans=", true_ans)
                print("true_ans=", true_ans)
                print(f"{true_ans} in {gen_txt}= {true_ans in gen_txt}")
                prompt_gen_acc = torch.FloatTensor([true_ans in gen_txt])
                print("prompt_gen_acc=", prompt_gen_acc)
...
def main(script_args):
    ...
    stop_strings, stop_tokens = get_stoppings(script_args.pmodel_name, tokenizer)
    print(stop_tokens)
    ...
    with open(script_args.out_path, 'w') as outf:
        for step, batch in tqdm(enumerate(prompt_dataloader), total=len(prompt_dataloader)):
            ...
            with torch.no_grad():
                ...
                cur_metrics = calc_all_metrics(ref_model, tokenizer, seq, seq_attention_mask, answer_input_ids, answer_attention_mask, stop_strings, stop_tokens)
