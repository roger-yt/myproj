...
class ScriptArguments:
    ...
    gradient_accumulation_steps: Optional[int] = field(default=16)  # ensure batch_size=128!!!
...
tokenizer = AutoTokenizer.from_pretrained(script_args.model_name) #AutoTokenizer
tokenizer.pad_token = tokenizer.eos_token

train_dataset = load_dataset(script_args.train_set_path, split="train").shuffle(seed=42)
column_names = list(train_dataset.features)
train_dataset = train_dataset.map(task_config.m_sft_tokenize(tokenizer), remove_columns=column_names, num_proc=16)
