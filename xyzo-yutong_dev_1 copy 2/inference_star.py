...
def run_inference_one_gpu(gpu_id, question_list, hint_question_list, answer_list, answer_num_list):
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    #os.environ["CUDA_VISIBLE_DEVICES"] = os.environ["CUDA_VISIBLE_DEVICES"].split(",")[gpu_id]
    return generate_rational(question_list, hint_question_list, answer_list, answer_num_list)

def run_inference_multi_gpu(questions, hint_questions, answers, answer_nums):
    split_questions = split_list(questions, NUM_GPUS)
    split_hint_questions = split_list(hint_questions, NUM_GPUS)
    split_answers = split_list(answers, NUM_GPUS)
    split_answer_nums = split_list(answer_nums, NUM_GPUS)
    inputs = [(i+GPUS[0], p, split_hint_questions[i], split_answers[i], split_answer_nums[i]) for i, p in enumerate(split_questions)]
...
if __name__ == "__main__":
    ...
    # for i in range(10):
    #     print("dataset_[{}]:".format(i), dataset_[i])
    # dataset_ = dataset_.select(range(10))
    questions = dataset_["template_question"]
    hint_questions = dataset_["hint_template_question"]
    ...
    def generate_rational(few_shot_questions, hint_questions, answers, answer_nums):
        ...
        hint_rational = llm.generate(hint_questions, sampling_params, use_tqdm=True)
        rational_answer = []
        for i, answer_text in enumerate(answers):
            rational_text = rational[i].outputs[0].text
            lab_ans = answer_nums[i]
            extract_ans = extract_answer(rational_text, data_name)[:len(lab_ans)]
            acc = math_equal(extract_ans, lab_ans)
            if acc == False:
                hint_rational_text = hint_rational[i].outputs[0].text
                extract_hint_ans = extract_answer(hint_rational_text, data_name)[:len(lab_ans)]
                acc = math_equal(extract_hint_ans, lab_ans)
                if acc:
                    rational_text = hint_rational_text
                    # print("hi")
                    # print(rational_text)
    ...
    rational_answer = run_inference_multi_gpu(questions, hint_questions, answers, answer_nums)
