...
dataset_path="ZhangShenao/code_opencoder_edu-deepseek-coder-1.3b-instruct-iter1_sample_120_tp"
split="[:${num_samples}]"
...
python m_sft_for_code.py --model_name "${company}/${model_name}" --train_set_path $dataset_path --output_dir $sft_model_dir    --Task_Type  "${task_pre}_${task_suf}" --learning_rate $sft_learning_rate --deepspeed deepspeed_configs/deepspeed_3.json --hub_model_id $sft_hub_id --gradient_accumulation_steps 1 --per_device_train_batch_size 1 --num_train_epochs 1
