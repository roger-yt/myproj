...
for i in $(seq 1 $iter_num); do
    ...
    CUDA_VISIBLE_DEVICES="${visible_devices}" python xiaojun_E_step_ent_PPO.py --model_name $e_input_model --critic_model_name $critic_model_name --task_type "${task_pre}_${task_suf}${split}" --model_path $e_model_dir || exit 1
    
    huggingface-cli upload "ZhangShenao/$e_hub_id" "${e_model_dir}/final_checkpoint" --token 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
    
    CUDA_VISIBLE_DEVICES="${visible_devices}" python inference_for_code.py --model_path "${e_model_dir}/final_checkpoint" --task_type "${task_pre}_${task_suf}" --dataset_path $dataset_path --dataset_fraction $split || exit 1
    CUDA_VISIBLE_DEVICES="${visible_devices}" python inference_xiaojun.py --model_path $e_input_model --dataset_path $dataset_path --save_prefix $m_model_dir --sft_data_type zq_raw --train_step $num_samples  --task_type  "${task_pre}_${task_suf}" --learning_rate $sft_learning_rate|| exit 1
