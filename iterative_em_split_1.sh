...
for i in $(seq 1 $iter_num); do
    ...
    huggingface-cli upload "ZhangShenao/$e_hub_id" "${e_model_dir}/tb_log" --token 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

    
    CUDA_VISIBLE_DEVICES="${visible_devices}" python inference.py --model_path "${e_model_dir}/final_checkpoint" --task_type "${task_pre}_${task_suf}" --dataset_path $dataset_path --dataset_fraction $split  --max_length $max_length|| exit 1
