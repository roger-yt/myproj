...
for i in $(seq 1 $iter_num); do
    ...
    path="./${model_name}-${task_suf}_sample_${num_samples}_nsks_ml${max_length}"
    mkdir $path
    e_input_model="${path}/m-iter-$((i-1))_zq_raw"
    e_model_dir="${path}/e-iter-$i"
    m_model_dir="${path}/m-iter-$i"
    e_hub_id="${task_pre}_${task_suf}-${model_name}-e-iter-${i}_sample_${num_samples}_nsks_ml${max_length}_mlr${m_lr}_ent${ent_coeff}"
    m_hub_id="${task_pre}_${task_suf}-${model_name}-m-iter-${i}_sample_${num_samples}_nsks_ml${max_length}_mlr${m_lr}_ent${ent_coeff}"
    dataset_path="YYT-t/${task_pre}_${task_suf}-${model_name}-iter${i}_sample_${num_samples}_nsks_ml${max_length}_mlr${m_lr}_ent${ent_coeff}"
    ...
    CUDA_VISIBLE_DEVICES="${visible_devices}" python xiaojun_E_step_ent_PPO_noskip_safe.py --model_name $e_input_model --critic_model_name $critic_model_name --task_type "${task_pre}_${task_suf}${split}" --model_path $e_model_dir  --max_length $max_length --model_max_length $model_max_length --ent_coeff $ent_coeff
