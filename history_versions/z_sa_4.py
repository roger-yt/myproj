    ...
    def compute_loss(self, model, inputs):
        with torch.no_grad():
            ...
            for i in range(len(rational_decode)):
                filtered_rational.append(rational_decode[i] + "\n")
            filtered_rational = tokenizer(filtered_rational, padding=True, return_tensors="pt")["input_ids"].to(inputs_ids_q.device)
            # print("filtered_rational: ", filtered_rational)
            ...
            #     if rational[i][-1] == 107:
            #         rational[i][-1] = 108
            z_y = torch.cat((filtered_rational,  inputs_ids_a), dim=1)
            print("z_y: ", z_y[0])
            # print("decode rational: ", tokenizer.decode(rational[0]))  # rational without final answer y
            print("decode z_y: ", tokenizer.decode(z_y[0]))
            # print("pure_rational: ", filtered_rational[0])
            # print("decode pure_rational: ", tokenizer.decode(filtered_rational[0]))
            # print("inputs_ids_q: ", inputs_ids_q[0])
            # print("decode inputs_ids_q: ", tokenizer.decode(inputs_ids_q[0]))
        ...
        loss = -reward * (model(filtered_rational, labels=filtered_rational)[0] - model(inputs_ids_q, labels=inputs_ids_q)[0])
        return loss
...
trainer.save_model(output_name + "/debug2")
tokenizer.save_pretrained(output_name + "/debug2")
