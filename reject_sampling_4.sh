...
dataset_path="ZhangShenao/rs_newb-${task_pre}_${task_suf}-${model_name}-iter${i}_sample_${num_samples}_temp_${temp}_gen_${gen_nums}_mlr${sft_learning_rate}"
split="[:${num_samples}]"
...
python inference_xiaojun.py --model_path "${company}/${model_name}" --dataset_path $dataset_path --save_prefix $rs_model_dir --sft_data_type zq_raw --train_step $num_samples  --task_type  "${task_pre}_${task_suf}" --learning_rate $sft_learning_rate|| exit 1
